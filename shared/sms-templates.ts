 ...
 *
 * This module lives in `shared/` on purpose: it is the SINGLE source of truth for
 * both the automatic senders (server) AND the manual "Send SMS" preview/send
 * (client → server). Never render parent SMS from Eskiz's own example text — that
 * text has a real name baked in (e.g. "Muhammad") with no placeholder, so sending
 * it verbatim would text every parent the same name. Always render from here.
 *
 ...
 * `ru-RU` uses a non-breaking / narrow-no-break space as the group separator
 * (which one depends on the ICU version); normalise every space-like separator
 * to a plain ASCII space so the sent text matches the approved Eskiz template
 * byte-for-byte and shows a normal space to parents.
 */
