...
export function accessFor(user: User): Access {
  ...
  // Front-desk roles whose home screen is Record Payment.
  const desk = accountant || assistant;
  ...
  return {
    ...
    // Front-desk roles record from the home screen; anyone else granted the
    // ability gets a dedicated Record page.
    record: ceo || can(user, "record_payment"),
    recordPath: desk ? "/" : "/record",
